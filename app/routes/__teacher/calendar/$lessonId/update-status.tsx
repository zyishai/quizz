...
export const loader = async ({ request, params }: LoaderArgs) => {
  ...
  if (userId) {
    ...
    if (teacher) {
      ...
      if (lesson.ended) {
        throw redirectBack(request, { fallback: "/calendar" });
      }
      const credits =
  ...
  return (
    <>
      ...
      <main className="mt-6 flex flex-1 flex-col overflow-hidden px-4 py-1">
        <h1 className="text-2xl font-bold text-gray-900">סיום שיעור</h1>
        {eventFetched && (
          <p className="mt-1 text-sm text-gray-500">
            סיום שיעור עבור {(lesson.student as Student).fullName} בתאריך{" "}
            {formatDate(lesson.event.dateAndTime)} בשעה{" "}
